...
if __name__=="__main__":
    ...
    if args.filtered_folder is not None:
        filtered_folder = args.filtered_folder

    else:
        logger.info("Use working directory to save filtered files.")
    ...
    vep_dict = configuration["Variant-Effect-Predictor"]
    ...
    if not is_filtered:
        logger.info(f"Perform prefiltering and save filtered files to following filtered folder: {filtered_folder}")
        annotate.call_vep_and_annotate_consequence_information(input_vcf, str(working_directory + "/" + input_filename + "_consequence.vcf"), vep_dict, annotation_dict, assembly_build, num_cores)
        ...
        logger.info(f"Skip prefiltering step and use filtered files found in the filtered folder: {filtered_folder}")
    ...
    annotate.call_vep_and_annotate_vcf(str(working_directory + "/" + input_filename + "_snp.vcf"), str(working_directory + "/" + input_filename + "_snp_vep.vcf"), vep_dict, annotation_dict, assembly_build, False, False, num_cores)
    annotate.call_vep_and_annotate_vcf(str(working_directory + "/" + input_filename + "_indel.vcf"), str(working_directory + "/" + input_filename + "_indel_vep.vcf"), vep_dict, annotation_dict, assembly_build, True, False, num_cores)
    annotate.call_vep_and_annotate_vcf(str(working_directory + "/" + input_filename + "_indel_expanded.vcf"), str(working_directory + "/" + input_filename + "_indel_expanded_vep.vcf"), vep_dict, annotation_dict, assembly_build, False, True, num_cores)

    # Convert annotated VCF files to pandas dataframes
    input_data_snp_annotated = convert_vcf.convert_vcf_to_pandas_dataframe(str(working_directory + "/" + input_filename + "_snp_vep.vcf"),allele_frequency_list , False, False, transcript_path, num_cores, CONSTANT_DICTIONARY)
    input_data_indel_annotated = convert_vcf.convert_vcf_to_pandas_dataframe(str(working_directory + "/" + input_filename + "_indel_vep.vcf"), allele_frequency_list, True, False, transcript_path, num_cores, CONSTANT_DICTIONARY)
    input_data_indel_expanded_annotated = convert_vcf.convert_vcf_to_pandas_dataframe(str(working_directory + "/" + input_filename + "_indel_expanded_vep.vcf"), allele_frequency_list, True, True, transcript_path, num_cores, CONSTANT_DICTIONARY)
    ...
    if compress_results:
        input_data_combined.to_csv(str(output_folder + "/" + input_filename + "_annotated.tsv.gz"), index=False, sep="\t", compression="gzip")
