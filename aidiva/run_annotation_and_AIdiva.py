...
if __name__=="__main__":
    ...
    parser.add_argument("--build", type=str, dest="build", metavar="GRCh37", required=False, help="Genome build to use [GRCh37, GRCh38]. (default: GRCh37)")
    ...
    annotate.sort_vcf(input_vcf, str(working_directory + input_filename + "_sorted.vcf"), vep_annotation_dict, build)
    annotate.annotate_consequence_information(str(working_directory + input_filename + "_sorted.vcf"), str(working_directory + input_filename + "_consequence.vcf"), vep_annotation_dict, build, num_cores)
    ...
    annotate.call_vep_and_annotate_vcf(str(working_directory + input_filename + "_snp.vcf"), str(working_directory + input_filename + "_snp_vep.vcf"), vep_annotation_dict, build, False, False, num_cores)
    annotate.call_vep_and_annotate_vcf(str(working_directory + input_filename + "_indel.vcf"), str(working_directory + input_filename + "_indel_vep.vcf"), vep_annotation_dict, build, True, False, num_cores)
    annotate.call_vep_and_annotate_vcf(str(working_directory + input_filename + "_indel_expanded.vcf"), str(working_directory + input_filename + "_indel_expanded_vep.vcf"), vep_annotation_dict, build, False, True, num_cores)
    ...
    annotate.annotate_from_vcf(str(working_directory + input_filename + "_snp_vep.vcf"), str(working_directory + input_filename + "_snp_vep_annotated.vcf"), vep_annotation_dict, False, False, build, num_cores)
    annotate.annotate_from_vcf(str(working_directory + input_filename + "_indel_vep.vcf"), str(working_directory + input_filename + "_indel_vep_annotated.vcf"), vep_annotation_dict, False, True, build, num_cores)
    annotate.annotate_from_vcf(str(working_directory + input_filename + "_indel_expanded_vep.vcf"), str(working_directory + input_filename + "_indel_expanded_vep_annotated.vcf"), vep_annotation_dict, True, False, build, num_cores)

    # TODO: add bed annotation
    annotate.annotate_from_bed(str(working_directory + input_filename + "_snp_vep_annotated.vcf"), str(working_directory + input_filename + "_snp_vep_annotated_bed.vcf"), vep_annotation_dict, build, num_cores)
    annotate.annotate_from_bed(str(working_directory + input_filename + "_indel_vep_annotated.vcf"), str(working_directory + input_filename + "_indel_vep_annotated_bed.vcf"), vep_annotation_dict, build, num_cores)

    # Filter low confidence regions
    annotate.filter_regions(str(working_directory + input_filename + "_snp_vep_annotated_bed.vcf"), str(working_directory + input_filename + "_snp_vep_annotated_bed_filtered.vcf"), vep_annotation_dict, build)
    annotate.filter_regions(str(working_directory + input_filename + "_indel_vep_annotated_bed.vcf"), str(working_directory + input_filename + "_indel_vep_annotated_bed_filtered.vcf"), vep_annotation_dict, build)
    ...
    # TODO: create new models for grch38
